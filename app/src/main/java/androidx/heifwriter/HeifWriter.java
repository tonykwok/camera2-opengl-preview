 ...
 *使用此类编写heif文件的一般顺序（伪代码）如下：
 * 1) Construct the writer://构建writer
 ...
 * heifwriter.start();
 *
 * 4) Depending on the chosen input mode, add one or more images using one of these methods://添加图像
 * heifwriter.addYuvBuffer(...);   Or
 * heifwriter.addBitmap(...);   Or
 * render to the previously obtained surface//渲染到先前获得的surface
 *
 * 5) Call stop:
 * heifwriter.stop(...);
 *
...
public final class HeifWriter implements AutoCloseable {
    ...
    public static final class Builder {
        ...
        /**
         * Construct a Builder with output specified by its path.//使用其路径指定的输出构造一个Builder。
         *
        ...
        /**
         * Construct a Builder with output specified by its file descriptor.//使用其文件描述符指定的输出构造一个Builder。
         *
        ...
        /**
         * Set the maximum number of images to write.设置要写入的最大图像数。
         *
